...
import io.debezium.connector.pgsql.connection.PostgresReplicationConnection;
import io.debezium.connector.pgsql.connection.ReplicationConnection;
import io.debezium.connector.pgsql.junit.SkipTestDependingOnDecoderPluginNameRule;
import io.debezium.connector.pgsql.junit.SkipWhenDecoderPluginNameIs;
import io.debezium.connector.pgsql.junit.SkipWhenDecoderPluginNameIsNot;
import io.debezium.connector.pgsql.snapshot.InitialOnlySnapshotter;
...
public class PostgresConnectorIT extends AbstractConnectorTest {
    ...
    public void shouldValidateConfiguration() throws Exception {
        ...
        validateConfigField(validatedConfig, PostgresConnectorConfig.PLUGIN_NAME, PostgresConnectorConfig.LogicalDecoder.DECODERBUFS.getValue());
        ...
        validateConfigField(validatedConfig, PostgresConnectorConfig.SNAPSHOT_MODE, PostgresConnectorConfig.SnapshotMode.INITIAL);
    ...
    public void shouldReceiveChangesForChangeColumnDefault() throws Exception {
        ...
        try {
            ...
            final String topicName = TestHelper.topicName("default_change.test_table");
    ...
    public void shouldNotIgnoreEventsForDeletedTable() throws Exception {
        ...
        assertThat(actualRecords.recordsForTopic(TestHelper.topicName("s1.a"))).hasSize(1);
        assertThat(actualRecords.recordsForTopic(TestHelper.topicName("s2.a"))).hasSize(1);
    }
    ...
    public void shouldNotProduceEventsWithInitialOnlySnapshot() throws InterruptedException {
        ...
        TestHelper.execute(SETUP_TABLES_STMT);
        Configuration config = TestHelper.defaultConfig()
                .with(PostgresConnectorConfig.SNAPSHOT_MODE, PostgresConnectorConfig.SnapshotMode.INITIAL_ONLY.getValue())
                .with(PostgresConnectorConfig.DROP_SLOT_ON_STOP, Boolean.TRUE)
                .build();
        start(PostgresConnector.class, config);
        assertConnectorIsRunning();
    ...
    public void shouldProduceEventsWhenAlwaysTakingSnapshots() throws InterruptedException {
        TestHelper.execute(SETUP_TABLES_STMT);
        Configuration.Builder configBuilder = TestHelper.defaultConfig()
                .with(PostgresConnectorConfig.SNAPSHOT_MODE, PostgresConnectorConfig.SnapshotMode.ALWAYS.getValue())
                .with(PostgresConnectorConfig.DROP_SLOT_ON_STOP, Boolean.FALSE);
        start(PostgresConnector.class, configBuilder.build());
        assertConnectorIsRunning();
        waitForSnapshotToBeCompleted();
    ...
    public void shouldTakeExcludeListFiltersIntoAccount() throws Exception {
        ...
        recordsForS1a.forEach(record -> {
            VerifyRecord.isValidRead(record, TestHelper.PK_FIELD, pkValue.getAndIncrement());
    ...
    public void shouldRemoveWhiteSpaceChars() throws Exception {
        ...
        List<SourceRecord> records = actualRecords.recordsForTopic(TestHelper.topicName("s1.b"));
    ...
    public void shouldCloseTxAfterTypeQuery() throws Exception {
        ...
        SourceRecord record = records.get(0);
        VerifyRecord.isValidInsert(record, TestHelper.PK_FIELD, 1);
    ...
    public void shouldRegularlyFlushLsnWithTxMonitoring() throws InterruptedException, SQLException {
        ...
        final String txTopic = TestHelper.topicName("transaction");
        TestHelper.execute(INSERT_STMT);
    ...
    public void shouldFlushLsnOnEmptyMessage() throws InterruptedException, SQLException {
        ...
        Awaitility.await().atMost(TestHelper.waitTimeForRecords(), TimeUnit.SECONDS).until(() -> {
            final SourceRecords actualRecords = consumeRecordsByTopic(1);
            final List<SourceRecord> topicRecords = actualRecords.recordsForTopic(TestHelper.topicName("s1.a"));
    ...
    public void shouldAllowForCustomSnapshot() throws InterruptedException {
        ...
        List<SourceRecord> s1recs = actualRecords.recordsForTopic(TestHelper.topicName("s1.a"));
        List<SourceRecord> s2recs = actualRecords.recordsForTopic(TestHelper.topicName("s2.a"));
        ...
        s1recs = actualRecords.recordsForTopic(TestHelper.topicName("s1.a"));
        s2recs = actualRecords.recordsForTopic(TestHelper.topicName("s2.a"));
        assertThat(s1recs.size()).isEqualTo(1);
        assertThat(s2recs.size()).isEqualTo(1);
        record = s1recs.get(0);
        VerifyRecord.isValidInsert(record, TestHelper.PK_FIELD, 2);
        record = s2recs.get(0);
        VerifyRecord.isValidInsert(record, TestHelper.PK_FIELD, 2);
        stopConnector();
        ...
        VerifyRecord.isValidRead(s1recs.get(0), TestHelper.PK_FIELD, 1);
        VerifyRecord.isValidRead(s1recs.get(1), TestHelper.PK_FIELD, 2);
        VerifyRecord.isValidRead(s2recs.get(0), TestHelper.PK_FIELD, 1);
        VerifyRecord.isValidRead(s2recs.get(1), TestHelper.PK_FIELD, 2);
    }
    ...
    public void shouldAllowForSelectiveSnapshot() throws InterruptedException {
        TestHelper.execute(SETUP_TABLES_STMT);
        Configuration.Builder configBuilder = TestHelper.defaultConfig()
                .with(PostgresConnectorConfig.SNAPSHOT_MODE, PostgresConnectorConfig.SnapshotMode.ALWAYS.name())
        ...
        assertThat(s1recs.size()).isEqualTo(1);
        assertThat(s2recs.size()).isEqualTo(1);
        VerifyRecord.isValidInsert(s1recs.get(0), TestHelper.PK_FIELD, 2);
        VerifyRecord.isValidInsert(s2recs.get(0), TestHelper.PK_FIELD, 2);
    ...
    public void shouldAllowForExportedSnapshot() throws Exception {
        ...
        TestHelper.execute(SETUP_TABLES_STMT);
        Configuration config = TestHelper.defaultConfig()
                .with(PostgresConnectorConfig.SNAPSHOT_MODE, PostgresConnectorConfig.SnapshotMode.EXPORTED.getValue())
                .with(PostgresConnectorConfig.DROP_SLOT_ON_STOP, Boolean.FALSE)
                .build();
        start(PostgresConnector.class, config);
        assertConnectorIsRunning();
        ...
        assertThat(s1recs.size()).isEqualTo(1);
        assertThat(s2recs.size()).isEqualTo(1);
        VerifyRecord.isValidInsert(s1recs.get(0), TestHelper.PK_FIELD, 3);
        VerifyRecord.isValidInsert(s2recs.get(0), TestHelper.PK_FIELD, 3);
    }
    ...
    public void exportedSnapshotShouldNotSkipRecordOfParallelTx() throws Exception {
        ...
        List<SourceRecord> s1recs = actualRecords.recordsForTopic(TestHelper.topicName("s1.a"));
        List<SourceRecord> s2recs = actualRecords.recordsForTopic(TestHelper.topicName("s1.a"));
        s2recs = actualRecords.recordsForTopic(TestHelper.topicName("s2.a"));
        assertThat(s1recs.size()).isEqualTo(2);
        assertThat(s2recs.size()).isEqualTo(2);
    }
    ...
    public void shouldResumeStreamingFromSlotPositionForCustomSnapshot() throws Exception {
        ...
        VerifyRecord.isValidRead(s1recs.get(1), TestHelper.PK_FIELD, 1);
        VerifyRecord.isValidRead(s1recs.get(2), TestHelper.PK_FIELD, 2);
        VerifyRecord.isValidRead(s2recs.get(1), TestHelper.PK_FIELD, 1);
        VerifyRecord.isValidRead(s2recs.get(2), TestHelper.PK_FIELD, 2);
    ...
    public void shouldRewriteIdentityKey() throws InterruptedException {
        ...
        records.recordsForTopic("test_server.s1.a").forEach(record -> {
            ...
            Assertions.assertThat(key.get(TestHelper.PK_FIELD)).isNotNull();
    ...
    public void shouldConsumeEventsWithMaskedColumns() throws Exception {
        ...
        List<SourceRecord> recordsForTopicS2 = actualRecords.recordsForTopic(TestHelper.topicName("s2.a"));
        ...
        recordsForTopicS2 = actualRecords.recordsForTopic(TestHelper.topicName("s2.a"));
        assertThat(recordsForTopicS2.size()).isEqualTo(1);
        ...
        record = recordsForTopicS2.remove(0);
        VerifyRecord.isValidUpdate(record, TestHelper.PK_FIELD, 2);
    ...
    public void shouldConsumeEventsWithMaskedHashedColumns() throws Exception {
        ...
        recordsForTopicS2 = actualRecords.recordsForTopic(TestHelper.topicName("s2.b"));
        assertThat(recordsForTopicS2.size()).isEqualTo(1);
    ...
    public void shouldUpdatePublicationForConfiguredTables() throws Exception {
        ...
        // there should be no record for s1.a
        List<SourceRecord> initalS1recs = actualRecords.recordsForTopic(TestHelper.topicName("s1.a"));
        List<SourceRecord> initalS2recs = actualRecords.recordsForTopic(TestHelper.topicName("s2.a"));
        assertThat(initalS1recs).isNull();
        assertThat(initalS2recs).hasSize(1);

        VerifyRecord.isValidInsert(initalS2recs.get(0), TestHelper.PK_FIELD, 2);
        ...
        List<SourceRecord> afterUpdateS1recs = actualRecordsAfterUpdate.recordsForTopic(TestHelper.topicName("s1.a"));
        List<SourceRecord> afterUpdateS2recs = actualRecordsAfterUpdate.recordsForTopic(TestHelper.topicName("s2.a"));
    ...
    public void shouldUpdateExistingPublicationForConfiguredPartitionedTables() throws Exception {
        ...
        List<SourceRecord> recs = actualRecords.recordsForTopic(TestHelper.topicName("s1.part"));
        List<SourceRecord> part1recs = actualRecords.recordsForTopic(TestHelper.topicName("s1.part1"));
        List<SourceRecord> part2recs = actualRecords.recordsForTopic(TestHelper.topicName("s1.part2"));
        assertThat(recs).hasSize(2);
        assertThat(part1recs).isNull();
        assertThat(part2recs).isNull();

        VerifyRecord.isValidInsert(recs.get(0), TestHelper.PK_FIELD, 1);
        VerifyRecord.isValidInsert(recs.get(1), TestHelper.PK_FIELD, 501);
    }
    ...
    public void shouldEmitNoEventsForSkippedCreateOperations() throws Exception {
        ...
        List<SourceRecord> recordsForTopic = records.recordsForTopic(TestHelper.topicName("s1.a"));

        assertThat(recordsForTopic.size()).isEqualTo(3);
        assertInsert(recordsForTopic.get(0), TestHelper.PK_FIELD, 201);
        assertInsert(recordsForTopic.get(1), TestHelper.PK_FIELD, 202);
        assertInsert(recordsForTopic.get(2), TestHelper.PK_FIELD, 203);
    ...
    public void testStreamingWithNumericReplicationSlotName() throws Exception {
        ...
        assertInsert(recordsForTopic.get(1), TestHelper.PK_FIELD, 201);
        assertInsert(recordsForTopic.get(2), TestHelper.PK_FIELD, 202);
        assertInsert(recordsForTopic.get(3), TestHelper.PK_FIELD, 203);
    }
    ...
    public void testShouldNotCloseConnectionFetchingMetadataWithNewDataTypes() throws Exception {
        ...
        SourceRecords records = consumeRecordsByTopic(1);
        List<SourceRecord> recordsForTopic = records.recordsForTopic(TestHelper.topicName("s1.c"));

        assertThat(recordsForTopic).hasSize(1);
        assertInsert(recordsForTopic.get(0), TestHelper.PK_FIELD, 1);
    ...
    public void shouldReselectToastColumnsOnPrimaryKeyChange() throws Exception {
        ...
        SourceRecords records = consumeRecordsByTopic(1);
        List<SourceRecord> recordsForTopic = records.recordsForTopic(TestHelper.topicName("s1.dbz5295"));
        assertThat(recordsForTopic).hasSize(1);
        ...
        recordsForTopic = records.recordsForTopic(TestHelper.topicName("s1.dbz5295"));
    ...
    private Predicate<SourceRecord> stopOnPKPredicate(int pkValue) {
        return record -> {
            Struct key = (Struct) record.key();
            return ((Integer) key.get(TestHelper.PK_FIELD)) == pkValue;
        };
    }
    ...
    private void assertRecordsFromSnapshot(int expectedCount, int... pks) throws InterruptedException {
        ...
        List<SourceRecord> recordsForTopicS1 = actualRecords.recordsForTopic(TestHelper.topicName("s1.a"));
        assertThat(recordsForTopicS1.size()).isEqualTo(expectedCountPerSchema);
        IntStream.range(0, expectedCountPerSchema)
                .forEach(i -> VerifyRecord.isValidRead(recordsForTopicS1.remove(0), TestHelper.PK_FIELD, pks[i]));

        List<SourceRecord> recordsForTopicS2 = actualRecords.recordsForTopic(TestHelper.topicName("s2.a"));
        assertThat(recordsForTopicS2.size()).isEqualTo(expectedCountPerSchema);
        IntStream.range(0, expectedCountPerSchema)
                .forEach(i -> VerifyRecord.isValidRead(recordsForTopicS2.remove(0), TestHelper.PK_FIELD, pks[i + expectedCountPerSchema]));
    }
    ...
    private void assertRecordsAfterInsert(int expectedCount, int... pks) throws InterruptedException {
        ...
        List<SourceRecord> recordsForTopicS1 = actualRecords.recordsForTopic(TestHelper.topicName("s1.a"));
        assertThat(recordsForTopicS1.size()).isEqualTo(expectedCountPerSchema);
        IntStream.range(0, expectedCountPerSchema).forEach(i -> VerifyRecord.isValidInsert(recordsForTopicS1.remove(0), TestHelper.PK_FIELD, pks[i]));

        List<SourceRecord> recordsForTopicS2 = actualRecords.recordsForTopic(TestHelper.topicName("s2.a"));
        assertThat(recordsForTopicS2.size()).isEqualTo(expectedCountPerSchema);
        IntStream.range(0, expectedCountPerSchema).forEach(i -> VerifyRecord.isValidInsert(recordsForTopicS2.remove(0), TestHelper.PK_FIELD, pks[i]));
    }
